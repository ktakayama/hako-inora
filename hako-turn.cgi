...
sub logLandInora {
	...
	logOut("${HtagName_}${name}島${H_tagName}の<b>$monsName</b>は、${HtagComName_}${comName}${H_tagComName}へ行きました。",$id);
END
}
...
sub logGrowUp {
	...
	logOut("${HtagName_}${name}島${H_tagName}の<b>${inoraName}</b>は${HtagComName_}${typeName}${H_tagComName}に進化しました！！",$id);
}
...
sub logGrowDown {
	my($id, $name, $typeName, $inoraName) = @_;
	$typeName = $HmonsterName[$typeName];
	logOut("${HtagName_}${name}島${H_tagName}の<b>${inoraName}</b>は${HtagComName_}${typeName}${H_tagComName}に退化しました？！",$id);
}

# いのらしゃべる
sub logSpeak {
	my($id, $name, $mName, $msg) = @_;
	logOut("${HtagName_}${name}島${H_tagName}:<b>${mName}</b> <FONT SIZE=+1>「${msg}」</FONT>",$id);
}
...
sub logMeteoMonster2 {
	...
	logOut("${HtagName_}${name}島$point${H_tagName}の<B>$tLname</B>に${HtagDisaster_}隕石${H_tagDisaster}が落下、<B>$tLname</B>は苦しそうに咆哮しました。",$id);
}
...
sub logContest {
	...
	logOut("${HtagName_}${name}島${H_tagName}の<b>$mName</b>が<B>$pName</B>で優勝しました。",$id);
	logHistory("${HtagName_}${name}島${H_tagName}の<b>$mName</b>が<B>$pName</B>で優勝");
}

# いのら睡眠中
sub logSleepInora {
	my($id, $name, $mName, $comName) = @_;
	logOut("${HtagName_}${name}島${H_tagName}で予定されていた${HtagComName_}$comName${H_tagComName}は、<B>$mName</B>が睡眠中だったため中止されました。",$id);
}

# 怪獣暴れる
sub logAngryQuakeMonster {
	my($id, $name, $mName) = @_;
	logOut("${HtagName_}${name}島${H_tagName}の<B>$mName</B>が暴れだしました。",$id);
}

# 怪獣機嫌悪し
sub logAngryMonster {
	my($id, $name, $mName) = @_;
	logOut("${HtagName_}${name}島${H_tagName}の<B>$mName</B>の${HtagComName_}機嫌が悪い${H_tagComName}様子です。",$id);
}

# 怪獣目覚める
sub logWakeUpMonster {
	my($id, $name, $mName) = @_;
	logOut("${HtagName_}${name}島${H_tagName}の<B>$mName</B>が目を覚ましました。",$id);
}

# 怪獣寝る
sub logSleepMonster {
	my($id, $name, $mName) = @_;
	logOut("${HtagName_}${name}島${H_tagName}の<B>$mName</B>が眠りにつきました。",$id);
}

# 怪獣病気
sub logSickMonster {
	my($id, $name, $mName, $sick) = @_;
	logOut("${HtagName_}${name}島${H_tagName}の<B>$mName</B>が${HtagDisaster_}$sick${H_tagDisaster}寝込みました！！",$id);
}

# 怪獣回復
sub logGenki {
	my($id, $name, $mName) = @_;
	logOut("${HtagName_}${name}島${H_tagName}の<B>$mName</B>は<b>元気</b>になりました。",$id);
}
...
sub tempNewIslandNoName {
	my($name) = $_[0];
	out(<<END);
${HtagBig_}${name}につける名前が必要です。${H_tagBig}$HtempBack
END
}
